...
export const appendHistoryEvent = async (
  ...
  let index = (await storage.get<HistoryIndex>(HISTORY_INDEX_KEY)) || defaultHistoryIndex();
  ...
  const chunk = (await storage.get<NebulaRoomEvent[]>(chunkKey)) || [];
  chunk.push(event);
  ...
  while (index.chunk - index.firstChunk + 1 > maxChunks) {
    ...
    const staleChunk = (await storage.get<NebulaRoomEvent[]>(staleKey)) || [];
 ...
 */
export const loadRecentHistoryEvents = async (
  storage: DurableObjectStorage,
  historyLimit: number,
  limit: number
): Promise<NebulaRoomEvent[]> => {
  const index = (await storage.get<HistoryIndex>(HISTORY_INDEX_KEY)) || defaultHistoryIndex();
  if (index.total === 0) {
    return [];
  }

  const need = Math.min(limit, historyLimit, index.total);
  const collected: NebulaRoomEvent[] = [];
  let cursor = index.chunk;

  while (cursor >= index.firstChunk && collected.length < need) {
    const chunk = (await storage.get<NebulaRoomEvent[]>(`${HISTORY_CHUNK_PREFIX}${cursor}`)) || [];
    if (chunk.length > 0) {
 ...
 */
export const loadHistoryIndex = async (storage: DurableObjectStorage): Promise<HistoryIndex> => {
  return (await storage.get<HistoryIndex>(HISTORY_INDEX_KEY)) || defaultHistoryIndex();
};
