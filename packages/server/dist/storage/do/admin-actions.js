 ...
 * 处理 `/admin/count` 请求。
 */
export const handleAdminCountRequest = (request, context) => {
 ...
 */
export const handleAdminRemoveUsersRequest = async (request, context) => {
    ...
    const adminReason = typeof body.reason === 'string'
        ? body.reason.trim()
        : 'removed by admin';
    const closeCode = Number.isFinite(body.code)
        ? Number(body.code)
        : 4003;
 ...
 * 处理 `/admin/disband` 请求。
 */
export const handleAdminDisbandRequest = async (request, context) => {
    if (request.method !== 'POST') {
        return Response.json({ error: 'method_not_allowed', message: '请使用 POST' }, { status: 405 });
    }
    const body = (await safeParseJson(request));
...
const resolveTargetUserIds = (body) => {
    const userIds = new Set();
    const many = body.userIds;
...
const safeParseJson = async (request) => {
    try {
