...
export class NebulaDurableObject {
    ...
    rateLimitDisconnectThreshold = 3;
    rateLimitViolationWindowMs = 60_000;
    idempotencyCacheSize = 500;
    ...
    async fetch(request) {
        ...
        const adminContext = {
            ...
            disconnect: (ws, leaveMeta) => this.handleDisconnect(ws, leaveMeta),
            onDisbandLog: (data) => this.log('warn', 'room.disband', data),
            onAdminAudit: (data) => this.log('info', 'admin.operation', data)
        };
        switch (url.pathname) {
            case '/ws':
            ...
            default:
                return Response.json({ error: 'invalid_room_action', message: '支持动作: /ws, /messages, /presence, /history, /stats, /admin/count, /admin/remove-users, /admin/disband' }, { status: 404 });
        }
    }
    async alarm() {
        const now = Date.now();
    ...
    async webSocketMessage(ws, message) {
        ...
        if (rateError) {
            ...
            const shouldDisconnect = registerRateLimitViolation(meta, this.rateLimitDisconnectThreshold, this.rateLimitViolationWindowMs);
            if (shouldDisconnect) {
