...
export class NebulaHandler {
    ...
    async handle(request, env) {
        ...
        const action = (roomMatch?.[2] || 'presence');
        const adminAction = (adminMatch?.[2] || null);
        if (adminAction) {
        ...
        headers.set('x-nebula-idempotency-cache-size', `${this.config.message?.idempotencyCacheSize ?? 500}`);
        headers.set('x-nebula-idempotency-ttl-seconds', `${this.config.message?.idempotencyTTLSeconds ?? 300}`);
        ...
        headers.set('x-nebula-rate-limit-disconnect-threshold', `${this.config.rateLimit?.disconnectThreshold ?? 3}`);
        headers.set('x-nebula-rate-limit-violation-window-ms', `${this.config.rateLimit?.violationWindowMs ?? 60_000}`);
