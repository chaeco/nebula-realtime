...
export type PublishResult =
  ...
  | { type: 'deduplicated'; response: Response }
  | { type: 'created'; response: Response; event: NebulaRoomEvent };
 ...
 */
export const processPublishRequest = async (input: ProcessPublishRequestInput): Promise<PublishResult> => {
  const parsed = await parseAndValidatePublishBody(
    input.request,
    input.maxEventNameLength,
  ...
  const idempotencyKey =
    input.request.headers.get('x-nebula-idempotency-key') || body.idempotencyKey;
  if (idempotencyKey) {
    const existing = await getIdempotentEvent(
      input.storage,
      idempotencyKey,
      input.idempotencyTTLSeconds,
      input.idempotencyCacheSize
    );
    if (existing) {
      return {
        type: 'deduplicated',
  ...
  const event = createRoomEvent(
    ...
    body.senderId || input.request.headers.get('x-nebula-user-id') || 'http',
    body.payload ?? null
  );
  await appendHistoryEvent(input.storage, event, input.historyLimit, input.historyChunkSize);
  if (idempotencyKey) {
    await storeIdempotentEvent(
      input.storage,
      idempotencyKey,
      event,
      input.idempotencyTTLSeconds,
