...
export const parseAndValidatePublishBody = async (
  ...
  | { ok: true; body: { event?: string; payload?: unknown; senderId?: string; idempotencyKey?: string } }
  | { ok: false; response: Response }
> => {
  let body: { event?: string; payload?: unknown; senderId?: string; idempotencyKey?: string };
  try {
    body = (await request.json()) as { event?: string; payload?: unknown; senderId?: string; idempotencyKey?: string };
  } catch {
    return {
      ok: false,
