...
import { handleAdminCountRequest, handleAdminDisbandRequest, handleAdminRemoveUsersRequest } from './do/admin-actions.js';
import { logStructured } from './do/observability.js';
import { parseIncomingWsPayload } from './do/parsers.js';
import { processPublishRequest } from './do/publish.js';
import { buildHistoryResponse, buildPresenceResponse, buildStatsResponse } from './do/queries.js';
import { checkConnectionRateLimit, registerRateLimitViolation } from './do/rate-limit.js';
import { buildSystemEnvelope, resolveRuntimeConfig } from './do/runtime-options.js';
import { ConnectionMeta, DEFAULT_HISTORY_CHUNK_SIZE, DurableRuntimeConfig, LeaveMeta, RuntimeMetrics, createRoomEvent, defaultMetrics } from './do/types.js';
export class NebulaDurableObject implements DurableObject {
  ...
  private rateLimitDisconnectThreshold = 3;
  private rateLimitViolationWindowMs = 60_000;
  private idempotencyCacheSize = 500;
  private idempotencyTTLSeconds = 300;
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    this.applyRuntimeOptions(resolveRuntimeConfig(request, {
      roomId: this.roomId, historyLimit: this.historyLimit, maxEventNameLength: this.maxEventNameLength,
      maxPayloadBytes: this.maxPayloadBytes, heartbeatIntervalMs: this.heartbeatIntervalMs,
      heartbeatTimeoutMs: this.heartbeatTimeoutMs, protocolVersion: this.protocolVersion,
      rateLimitPerSecond: this.rateLimitPerSecond, rateLimitPerMinute: this.rateLimitPerMinute,
      rateLimitDisconnectThreshold: this.rateLimitDisconnectThreshold,
      rateLimitViolationWindowMs: this.rateLimitViolationWindowMs, idempotencyCacheSize: this.idempotencyCacheSize,
      idempotencyTTLSeconds: this.idempotencyTTLSeconds, structuredLogs: this.structuredLogs,
    ...
    const adminContext = {
      ...
      disconnect: (ws: WebSocket, leaveMeta: LeaveMeta) => this.handleDisconnect(ws, leaveMeta),
      onDisbandLog: (data: { disconnected: number; reason: string; code: number }) =>
        this.log('warn', 'room.disband', data),
      onAdminAudit: (data: {
      ...
      }) => this.log('info', 'admin.operation', data)
    };
    switch (url.pathname) {
      case '/ws':
      ...
      case '/admin/count':
        return handleAdminCountRequest(request, adminContext);
      case '/admin/remove-users':
        return handleAdminRemoveUsersRequest(request, adminContext);
      case '/admin/disband':
        return handleAdminDisbandRequest(request, adminContext);
      default:
        return Response.json(
          { error: 'invalid_room_action', message: '支持动作: /ws, /messages, /presence, /history, /stats, /admin/count, /admin/remove-users, /admin/disband' },
          { status: 404 }
        );
    }
  }
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    if (rateError) {
      ...
      const shouldDisconnect = registerRateLimitViolation(
        meta,
        this.rateLimitDisconnectThreshold,
        this.rateLimitViolationWindowMs
      );
      if (shouldDisconnect) {
  ...
  private async handleHttpPublish(request: Request): Promise<Response> {
    ...
    const result = await processPublishRequest({ request, storage: this.state.storage, roomId: this.roomId, protocolVersion: this.protocolVersion, maxEventNameLength: this.maxEventNameLength, maxPayloadBytes: this.maxPayloadBytes, historyLimit: this.historyLimit, historyChunkSize: this.historyChunkSize, idempotencyCacheSize: this.idempotencyCacheSize, idempotencyTTLSeconds: this.idempotencyTTLSeconds });
  ...
  private applyRuntimeOptions(config: DurableRuntimeConfig): void {
    ...
    this.rateLimitDisconnectThreshold = config.rateLimitDisconnectThreshold;
    this.rateLimitViolationWindowMs = config.rateLimitViolationWindowMs;
    this.idempotencyCacheSize = config.idempotencyCacheSize;
