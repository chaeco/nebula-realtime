import { NebulaAdminAction, NebulaConfig, Env } from '../types/index.js';
...
export class NebulaHandler {
   ...
   */
  constructor(private config: NebulaConfig) {}
  ...
  async handle(request: Request, env: Env): Promise<Response> {
    ...
    if (relativePath === '/' || relativePath === '/health') {
      return Response.json({
        ...
        endpoints: [
          ...
          `${normalizedPrefix}/rooms/:roomId/admin/count`,
          `${normalizedPrefix}/rooms/:roomId/admin/remove-users`,
          `${normalizedPrefix}/rooms/:roomId/admin/disband`
        ]
      });
    }

    const roomMatch = relativePath.match(/^\/rooms\/([^/]+)(?:\/(ws|messages|presence|history|stats))?\/?$/);
    const adminMatch = relativePath.match(/^\/rooms\/([^/]+)\/admin\/(count|remove-users|disband)\/?$/);
    if (!roomMatch && !adminMatch) {
      return Response.json(
        {
          error: 'invalid_route',
          message: '使用 /rooms/:roomId/(ws|messages|presence|history|stats|admin/count|admin/remove-users|admin/disband)'
        },
        { status: 404 }
      );
    }

    const roomId = decodeURIComponent((adminMatch || roomMatch)![1]);
    const action = (roomMatch?.[2] || 'presence') as
      | 'ws'
      | 'messages'
      | 'presence'
      | 'history'
      | 'stats';
    const adminAction = (adminMatch?.[2] || null) as NebulaAdminAction | null;

    if (adminAction) {
      const allowed = await this.config.authorizeAdmin?.({
        request,
        roomId,
        action: adminAction
      });
      if (!allowed) {
        return Response.json({ error: 'forbidden', message: 'admin operation is not allowed' }, { status: 403 });
      }
    }

    const resolvedUserId = resolveUserId(request);
    const allowAnonymous = this.config.allowAnonymous ?? true;
    if (!adminAction && !allowAnonymous && !resolvedUserId) {
      return Response.json(
    ...
    forwardUrl.pathname = adminAction ? `/admin/${adminAction}` : `/${action}`;
    ...
    headers.set(
      ...
      `${this.config.message?.maxEventNameLength ?? 64}`
    );
    headers.set(
      'x-nebula-message-max-payload-bytes',
      `${this.config.message?.maxPayloadBytes ?? 16 * 1024}`
    );
    headers.set(
      'x-nebula-idempotency-cache-size',
      `${this.config.message?.idempotencyCacheSize ?? 500}`
    );
    headers.set(
      'x-nebula-idempotency-ttl-seconds',
      `${this.config.message?.idempotencyTTLSeconds ?? 300}`
    );
    ...
    headers.set(
      ...
      `${this.config.rateLimit?.perConnectionPerSecond ?? 30}`
    );
    headers.set(
      'x-nebula-rate-limit-minute',
      `${this.config.rateLimit?.perConnectionPerMinute ?? 600}`
    );
    headers.set(
      'x-nebula-rate-limit-disconnect-threshold',
      `${this.config.rateLimit?.disconnectThreshold ?? 3}`
    );
    headers.set(
      'x-nebula-rate-limit-violation-window-ms',
      `${this.config.rateLimit?.violationWindowMs ?? 60_000}`
    );
    headers.set(
      'x-nebula-structured-logs',
      `${this.config.observability?.structuredLogs ?? true}`
    );
