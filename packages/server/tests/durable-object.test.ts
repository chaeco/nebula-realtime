...
describe('NebulaDurableObject HTTP APIs', () => {
  ...
  it('超限消息会触发 rate_limit 离开事件', async () => {
    ...
    (instance as any).rateLimitDisconnectThreshold = 3;
    (instance as any).connections.set(fakeWs, {
      userId: 'u-limit',
      lastSeenAt: Date.now(),
    ...
    await instance.webSocketMessage(fakeWs, JSON.stringify({ event: 'chat.message', payload: { n: 3 } }));
    await instance.webSocketMessage(fakeWs, JSON.stringify({ event: 'chat.message', payload: { n: 4 } }));
  ...
  it('相同幂等键重复发布会去重', async () => {
    const { state } = createState();
    const instance = new NebulaDurableObject(state as unknown as DurableObjectState);
    const headers = new Headers({
      'x-nebula-room-id': 'room-1',
      'x-nebula-idempotency-cache-size': '100',
      'x-nebula-idempotency-ttl-seconds': '300',
      'x-nebula-idempotency-key': 'k-1'
    });
    ...
    const first = await instance.fetch(
      new Request('https://example.com/messages', {
        method: 'POST',
        headers,
        body: JSON.stringify({ event: 'chat.message', payload: { text: 'same' } })
      })
    );
    ...
    const firstBody = (await first.json()) as { event: { id: string } };
    const secondBody = (await second.json()) as { event: { id: string }; deduplicated?: boolean };
